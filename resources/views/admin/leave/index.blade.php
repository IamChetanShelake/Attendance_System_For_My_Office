            ...
            <div class="section-card mb-4">
                <div class="card-header bg-gradient-primary text-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 d-flex align-items-center">
                        ...
                        <span class="badge bg-light text-dark ms-2">{{ $leaves->total() }}</span>
                    </h5>
                    <div class="d-flex gap-2">
                        <a href="{{ route('admin.employee-leave.create') }}" class="btn btn-light btn-lg">
                ...
                <div class="collapse show" id="filterSection">
                    <div class="card-body border-bottom bg-light">
                        <form method="GET" action="{{ route('admin.employee-leave.index') }}" class="row g-3" id="filterForm">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-12 text-end">
                                <a href="{{ route('admin.employee-leave.index') }}" class="btn btn-outline-secondary btn-sm">
                ...
                <!-- Bulk Actions -->
                <form method="POST" action="{{ route('admin.employee-leave.bulk-action') }}" id="bulkActionForm">
                    @csrf
                    <div class="card-body border-bottom bg-light d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <small class="text-muted">
                                Showing {{ $leaves->firstItem() }}-{{ $leaves->lastItem() }} of {{ $leaves->total() }} results
                            </small>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    @forelse($leaves as $leave)
                                        <tr class="position-relative">
                                            ...
                                            <td>
                                                <span class="badge bg-secondary">{{ $leave->formatted_leave_type }}</span>
                                                @if($leave->emergency)
                                            ...
                                            <td>
                                                <span class="fw-bold text-primary">{{ $leave->days_count }}</span>
                                                <small class="d-block text-muted">{{ $leave->is_half_day ? 'half day' : 'day' . ($leave->days_count > 1 ? 's' : '') }}</small>
                                            </td>
                                            <td>
                                                <div class="small">
                                                    <span class="fw-bold">{{ $leave->date_range }}</span>
                                                </div>
                                                @if($leave->approved_at)
                                                    <br><small class="text-muted">Approved: {{ $leave->approved_at->format('M j, Y') }}</small>
                                                @endif
                                            </td>
                                            <td>{!! $leave->status_badge !!}</td>
                                            <td>
                                                <div class="small">
                                                    {{ $leave->submitted_at ? $leave->submitted_at->format('M j, Y') : 'N/A' }}
                                                    <br><small class="text-muted">{{ $leave->submitted_at ? $leave->submitted_at->diffForHumans() : '' }}</small>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <a href="{{ route('admin.employee-leave.show', $leave) }}"
                                                       class="btn btn-outline-primary btn-sm"
                                                       title="View Details">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    <a href="{{ route('admin.employee-leave.edit', $leave) }}"
                                                    ...
                                                    @if($leave->status === 'pending')
                                                        <button type="button" class="btn btn-outline-success btn-sm"
                                                                onclick="showApprovalModal('{{ $leave->id }}', 'approve', '{{ $leave->employee->full_name }}')"
                                                                title="Approve Leave">
                                                            <i class="fas fa-check"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-outline-danger btn-sm"
                                                                onclick="showApprovalModal('{{ $leave->id }}', 'reject', '{{ $leave->employee->full_name }}')"
                                                                title="Reject Leave">
                                                    ...
                                                    @if($leave->status === 'approved' || $leave->status === 'rejected')
                                                        <button type="button" class="btn btn-outline-info btn-sm"
                                                                onclick="printLeaveRecord('{{ $leave->id }}')"
                                                                title="Print Record">
                                        ...
                                        <tr>
                                            <td colspan="8" class="text-center py-5">
                                                ...
                                                <p class="text-muted mb-4">
                                                    @if(request()->has('status') || request()->has('leave_type') || request()->has('employee_id'))
                                                        No records match your filter criteria.
                                                        <a href="{{ route('admin.employee-leave.index') }}" class="ms-1">Clear filters</a>
                                                    @else
                                                        No leave records have been added yet.
                                                    @endif
                                                </p>
                                                <a href="{{ route('admin.employee-leave.create') }}" class="btn btn-primary">
