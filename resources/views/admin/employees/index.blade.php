        ...
        <!-- Employee Management Section -->
            <div class="section-card">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <a href="{{ route('admin.employees.create') }}" class="btn btn-custom">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($employee->photo)
                                            <img src="{{ asset('storage/' . $employee->photo) }}" alt="Employee Photo" class="rounded-circle" style="width: 40px; height: 40px; object-fit: cover;">
                                        @else
                                    ...
                                    <td>
                                        <span class="badge bg-{{ $employee->type === 'onrole' ? 'primary' : 'info' }}">
                                    ...
                                    <td>
                                        @if($employee->probation_start_date && $employee->probation_end_date)
                                            <div class="small">
                                                <div>{{ $employee->probation_start_date->format('d M Y') }}</div>
                                                <div class="text-muted">to</div>
                                                <div>{{ $employee->probation_end_date->format('d M Y') }}</div>
                                                @if($employee->probation_status)
                                                    <span class="badge bg-{{ $employee->probation_status === 'active' ? 'warning' : ($employee->probation_status === 'completed' ? 'success' : ($employee->probation_status === 'extended' ? 'info' : 'danger')) }} mt-1">
                                                        {{ ucfirst($employee->probation_status) }}
                                                    </span>
                                                @endif
                                            </div>
                                            <button class="btn btn-sm btn-outline-primary mt-1 probation-btn" data-employee-id="{{ $employee->id }}" data-employee-name="{{ $employee->full_name }}" data-start-date="{{ $employee->probation_start_date ? $employee->probation_start_date->format('Y-m-d') : '' }}" data-end-date="{{ $employee->probation_end_date ? $employee->probation_end_date->format('Y-m-d') : '' }}" data-status="{{ $employee->probation_status }}" title="Manage Probation">
                                                <i class="fas fa-plus"></i>
                                            </button>
                                        @else
                                            <span class="text-muted">Not set</span>
                                            <button class="btn btn-sm btn-outline-success ms-2 probation-btn" data-employee-id="{{ $employee->id }}" data-employee-name="{{ $employee->full_name }}" data-start-date="" data-end-date="" data-status="" title="Set Probation Period">
                                                <i class="fas fa-plus"></i>
                                            </button>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.employees.show', $employee->id) }}" class="btn btn-sm btn-outline-info me-1" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('admin.employees.edit', $employee->id) }}" class="btn btn-sm btn-outline-custom me-1" title="Edit">
...
document.getElementById('probationForm').addEventListener('submit', function(e) {
    ...
    fetch('{{ route("admin.employees.update-probation") }}', {
        method: 'POST',
        body: formData,
        headers: {
